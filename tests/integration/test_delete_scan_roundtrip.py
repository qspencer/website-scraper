...
def test_delete_scan_cascades_everything(_mongo_reachable):
    ...
    try:
        ...
        # Guarantee no fixture data leaks into the live DB if an assertion above
        # failed before the cascade delete ran.
        if not deleted:
            mongodb_service.delete_scan(scan_url)
            history_service.delete_scan_by_url(scan_url)
